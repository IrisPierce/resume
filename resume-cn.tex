...
实验型依赖类型编程语言，支持一等公民的模式匹配和行多态
\begin{itemize}
  \item 支持依赖类型，自动求解变量以及非依赖的行多态的 Record 和 Sum 类型。
...
\begin{itemize}[parsep=0.25ex]
  ...
  \item \textbf{Kotlin/Java}:
    ...
    收录，熟悉 JNI、Gradle、Swing，
    有使用 Kotlin 编译器作为代码分析工具的经验

  \item \textbf{类型系统}:
    掌握依赖类型、（余）代数和 Martin-L\"{o}f 类型论，
    目前正在学习 Cubical 类型论，
    熟悉 Idris, Agda (\textbf{1 年}使用经验，开发组成员之一)
    和 F$\star$ 编程语言
    \subitem 读过代码的项目： Agda, Idris, MiniAgda, Mini-TT, TOG
  ...
  \item \textbf{IDE 工具开发}:
    ...
    熟悉 IntelliJ 平台的基础设施和整体架构（开发了
      ...
      \href{https://plugins.jetbrains.com/plugin/12176}
           {Dependently-Typed Lambda Calculus 插件},
...
\begin{itemize}[parsep=0.25ex]
  ...
  \item Crates.io: \url{https://crates.io/users/ice1000}, 发布了一些有趣的 Rust 库
  ...
  \item 开源贡献: \url{https://ice1000.org/opensource-contributions/}
    % 是 \textsf{JuliaEditorSupport}、\textsf{agda}、\textsf{pest-parser}、\textsf{EmmyLua} 等组织的成员，
    % 除了这些组织的项目之外还向 \textsf{KaTeX, shields.io, djinni, raft-rs, grpc-rs, intellij-solidity,
      % intellij-haskell, intellij-rust} 等项目贡献过代码
  ...
  \item 在
    ...
    上，以 Haskell、Agda 和 Idris 为主，达到 \textbf{1 dan}，全站排名 \#32
  \item 很喜欢交朋友
\end{itemize}
